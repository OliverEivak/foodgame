...
public class GameWonScreen implements Screen {
    ...
    public GameWonScreen(final App game) {
        this.game = game;
    }

    public GameWonScreen(final App game, int score) {
        this.game = game;

        stage = new Stage(new FitViewport(App.GAME_WIDTH, App.GAME_HEIGHT));
        renderer = new GameWonRenderer(stage, score);
        controller = new GameWonController(stage, renderer, game);
    }
